defmodule Mix.Tasks.Supabase.Gen.Auth do
  ...
  * `test/support/conn_case.exs` - The test helper for the authentication, modifies the existing one in-place.
  ...
  if Code.ensure_loaded?(Phoenix) and Code.ensure_loaded?(Mix.Phoenix) do
    ...
    def run(_args) do
      Mix.raise("""
      mix supabase.gen.auth requires Phoenix to be available.
      Please add Phoenix to your dependencies and try again.
      """)
    end
  end
end
